...
# 2. Read Priority Outreach Backlog
res = service.spreadsheets().values().get(spreadsheetId=spreadsheet_id, range="'Priority Outreach Backlog'!A1:J110").execute()
rows = res.get('values', [])
...
for idx, r in enumerate(rows[1:], start=2):
    ...
    if match_data and curr_st.lower() != match_data['status'].lower():
        ...
        up_note = f"{notes.strip()} | Applied 2026-08-25: {match_data['notes']}".strip(" | ")
        ...
        print(f"Row {idx:2d}: {company} -> {up_st}")

if batch_data:
    print(f"\nExecuting batch update for {len(batch_data)//2} rows in 'Priority Outreach Backlog'...")
    body = {'valueInputOption': 'RAW', 'data': batch_data}
    service.spreadsheets().values().batchUpdate(spreadsheetId=spreadsheet_id, body=body).execute()
    print("Priority Outreach Backlog 100% updated in batch!")
else:
    print("Priority Outreach Backlog was already 100% in sync.")
