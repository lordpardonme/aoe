...
for idx, r in enumerate(rows, start=2):
    ...
    if st.lower() == 'to contact' and email.strip():
        ...
        to_contact_by_country[c_name] = to_contact_by_country.get(c_name, 0) + 1

print("=== OVERALL STATUS BREAKDOWN ===")
for st, cnt in sorted(status_counts.items(), key=lambda x: x[1], reverse=True):
    print(f"• {st}: {cnt} rows")

print(f"\n=== REMAINING 'TO CONTACT' BY REGION ({sum(to_contact_by_country.values())} TOTAL) ===")
for c, cnt in sorted(to_contact_by_country.items(), key=lambda x: x[1], reverse=True):
    print(f"• {c}: {cnt} leads")
