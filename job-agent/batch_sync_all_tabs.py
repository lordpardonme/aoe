...
print(f"Loaded {len(applied_companies)} applied companies from Master Job Tracker.\n")
    ...
    try:
        ...
        for idx, r in enumerate(rows[1:], start=2):
            ...
            if match_data and curr_st.lower() != match_data['status'].lower():
                ...
                batch_data.append({
                    'range': f"'{tab}'!E{idx}:H{idx}",
                    'values': [[update_status, update_date, update_follow, update_notes]]
                })
        
        if batch_data:
            print(f"[{tab}] Found {len(batch_data)} rows to update. Executing 1 batch update...")
            body = {
                'valueInputOption': 'RAW',
                'data': batch_data
            }
            service.spreadsheets().values().batchUpdate(spreadsheetId=spreadsheet_id, body=body).execute()
            print(f"[{tab}] Successfully updated {len(batch_data)} rows in batch!")
        else:
            print(f"[{tab}] Already fully in sync.")
            
        time.sleep(1) # respectful pause
        
    except Exception as ex:
        print(f"Error on tab {tab}: {ex}")

print("\nALL BACKLOG TABS 100% IN SYNC VIA BATCH UPDATE!")
