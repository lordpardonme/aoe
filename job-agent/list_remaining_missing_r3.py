...
for i, r in enumerate(rows, 2):
    ...
    if status.strip().lower() == 'to contact' and (not em or any(k in em.lower() for k in ['find', 'linkedin', 'website', 'portal', 'n/a', 'none', '[']) or '@' not in em):
        still_missing.append({'row': i, 'company': comp.strip(), 'country': country.strip(), 'type': l_type.strip(), 'notes': notes.strip()})

print(f"Total remaining companies without email: {len(still_missing)}")
for idx, x in enumerate(still_missing, 1):
    print(f"{idx:3d}. Row {x['row']:3d} | {x['company']:<40} | {x['country']:<15}")
